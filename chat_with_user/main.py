...
# import database.work_with_db
# from database.work_with_db import BotUser, session
...
# token_bot = input('Token: ')
# Создание основного класса для работы (авторизация от сообщества)
...
def ask_for_info(person_id, lacking_params):
    params = {'city': 'город', 'sex': 'пол', 'bdate': 'год рождения', 'relation': 'семейное положение'}
    ...
    write_msg_bot(person_id, "Напишете какой параметр хотите указать и его значение, например "
                             "напечатайте команду (без пробела после знака равно): город=Санкт-Петербург. "
                             "При вводе пола, команда пол=1 значит, что ваш пол женский, пол=2 значит ваш пол мужской")
    write_msg_bot(person_id, "Для ввода семейного положения введите команду: семейное положение=1. "
            ...
            if event.to_me:
                ...
                if request == "привет":
                    ...
                    if event.user_id not in list_of_users:
                        ...
                        params_for_search = vk_user.identify_search_parametres(info_about_user)[0]
                        missing_params = vk_user.identify_search_parametres(info_about_user)[1]
                    ...
                    if missing_params:
                        ...
                        for numb in range(0, len(free_ids)):
                            ...
                            person_id = changed_photos[0]
                            link = 'https://vk.com/id' + str(person_id)
                            changed_photos.pop(0)
                            str_photos_link = ''
                            for one_elem in changed_photos:
                                photo_link = 'photo' + str(person_id) + '_' + str(one_elem)
                                if str_photos_link:
                    ...
                    parameters = {'город': 'city', 'пол': 'sex', 'год рождения': 'bdate',
